<?php if (preg_match('/\[(.*?)\]/', $id, $matches)) : ?>
    <?php
        ...
        $secondKey = $matches[1];
    ?>
    <select name="cashbene_gateway_plugin_settings[<?php echo esc_attr($firstKey) ?>][<?php echo esc_attr($secondKey) ?>]"  id="<?php echo esc_attr($id ?? "") ?>">
        <option value="">-</option>
        ...
        <?php endforeach; ?>
<?php else : ?>
    <select name="cashbene_gateway_plugin_settings[<?php echo esc_attr($id ?? "") ?>]"  id="<?php echo esc_attr($id ?? "") ?>">
        <option value="">-</option>
        <?php foreach($selectOptions as $value => $name) : ?>
