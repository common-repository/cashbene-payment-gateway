...
namespace Cashbene\GatewayWordpress\App\Utils;

use Cashbene\Core\Dto\Shop\Attribute;
...
final class Shop {
	...
	static public function getProduct($wcProduct, $country = 'pl'): Product {
		...
		$cashbenePrice = new Price();
		$cashbenePrice->amount = $wcProduct->get_price();
		$cashbenePrice->currency = strtoupper(get_woocommerce_currency());
		$cashbenePrice->setTaxEnabled(wc_tax_enabled());
		$cashbenePrice->setHasTax(wc_prices_include_tax());

		if(wc_tax_enabled()) {
			$taxArray = \WC_Tax::find_rates(['country' => $country]);
			if($taxArray) {
				$tax = reset($taxArray);
				$cashbenePrice->setTaxRate($tax['rate']);
			}
		}

		$cashbeneProduct = new Product();
		$cashbeneProduct->sku = $wcProduct->get_sku() ?: (string) $wcProduct->get_id();
		$cashbeneProduct->unitPrice = $cashbenePrice;
		$cashbeneProduct->name = $wcProduct->get_name();
		$cashbeneProduct->externalProductId = $wcProduct->get_id();
		$cashbeneProduct->image = wp_get_attachment_url($wcProduct->get_image_id()) ?? null;

        $cashbeneProduct->unitGrossPrice = clone $cashbenePrice;
        $cashbeneProduct->unitGrossPrice->amount = wc_get_price_including_tax($wcProduct);
        $cashbeneProduct->unitGrossPrice->setHasTax(true);

		if($attributes) {
			$cashbeneProduct->setAllAttributes($attributes);
			$cashbeneProduct->attributes = SerializerService::getSerializer()->denormalize($attributes, Attribute::class.'[]','array');
		}

		if($wcProduct->get_type() === 'variation') {
			$cashbeneProduct->productId = $wcProduct->get_parent_id();
		} else {
			$cashbeneProduct->productId = $wcProduct->get_id();
		}

		return $cashbeneProduct;
	}
	 ...
	 * @return false|string
	 */
    public static function getWcStatusByCashbeneStatus(string $status)
    {
        switch ($status) {
//            case "pending":
//                return "pending";
